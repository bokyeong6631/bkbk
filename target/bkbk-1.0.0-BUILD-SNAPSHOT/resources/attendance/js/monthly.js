		...
		function setMonthly(month, year) {
			...
			for(var dayNumber = 1; dayNumber <= dayQty; dayNumber++) {
				...
				var isInPast = options.stylePast && (
					...
					innerMarkup = '<div class="monthly-day-number" id="dateDiv">' + dayNumber + '</div><div class="monthly-indicator-wrap"></div>';
...
//					var dayQty = daysInMonth(month, year);
					// Otherwise, select the date ...
					$(String(options.target)).val(formatDate(setYear, setMonth, whichDay));
					// ... and then hide the calendar if it started that way
					if(today == whichDay){
						$(".monthly-today").append('<img src="/bkbk/res/images/check2.png" width="55" height="55" class="thumb"/>');
					}else if(today > whichDay){
						alert("이미 날짜가 지났습니다!");
					}else if(today < whichDay){
						alert("아직 찍을 수 없습니다!");
					}
